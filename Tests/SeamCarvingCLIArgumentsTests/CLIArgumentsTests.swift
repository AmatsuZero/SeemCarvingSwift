import XCTest
@testable import SeamCarvingCLIArguments
...
final class CLIOptionsTests: XCTestCase {
    ...
    func testMissingPathsThrow() {
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["--width", "20", "--height", "18"]))
    }

    func testInvalidDimensionsThrow() {
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--width", "0", "--height", "18"]))
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--width", "abc", "--height", "18"]))
    }

    func testMissingOneDimensionThrows() {
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--width", "20"]))
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--height", "18"]))
    }

    func testUnknownBackendThrows() {
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--width", "20", "--height", "18", "--backend", "gpu"]))
    }

    func testUnknownEnergyThrows() {
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--width", "20", "--height", "18", "--energy", "sideways"]))
    }

    func testForwardEnergyParses() throws {
        let options = try CLIArgumentParser.parseOptions(["in", "out", "--width", "20", "--height", "18", "--energy", "forward", "--backend", "metal"])
    ...
    func testFaceOptionsParse() throws {
        ...
        XCTAssertEqual(options.facePolicy, .vision)
        XCTAssertEqual(options.faceCadence, .eachPass)
    }

    // MARK: - Negative: unknown flags and duplicate positionals

    func testUnknownFlagThrows() {
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--width", "20", "--height", "18", "--nonsense"]))
    }

    func testDuplicatePositionalThrows() {
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "extra", "--width", "20", "--height", "18"]))
    }

    // MARK: - Negative: illegal numeric values

    func testIllegalFloatValuesThrow() {
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--percentage", "abc"]))
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--percentage", "-5"]))
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--width", "20", "--height", "18", "--blur-radius", "abc"]))
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--width", "20", "--height", "18", "--blur-radius", "-1"]))
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--width", "20", "--height", "18", "--sobel-threshold", "nan"]))
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--width", "20", "--height", "18", "--sobel-threshold", "-0.5"]))
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--width", "20", "--height", "18", "--protect-weight", "abc"]))
    }

    // MARK: - Negative: conflicting resize modes

    func testSquareConflictsWithExactDimensions() {
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--square", "--width", "20", "--height", "18"]))
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--square", "--width", "20"]))
    }

    func testPercentageConflictsWithExactDimensions() {
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--percentage", "50", "--width", "20", "--height", "18"]))
    }

    func testSquareConflictsWithPercentage() {
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--square", "--percentage", "50"]))
    }

    // MARK: - Reserved typed fields

    func testPercentageModeParses() throws {
        let options = try CLIArgumentParser.parseOptions(["in", "out", "--percentage", "50"])
        XCTAssertEqual(options.resizeMode, .percentage(50))
    }

    func testSquareModeParses() throws {
        let options = try CLIArgumentParser.parseOptions(["in", "out", "--square"])
        XCTAssertEqual(options.resizeMode, .square)
    }

    func testWidthHeightCompatibility() throws {
        let exact = try CLIArgumentParser.parseOptions(["in", "out", "--width", "20", "--height", "18"])
        XCTAssertEqual(exact.width, 20)
        XCTAssertEqual(exact.height, 18)

        let percentage = try CLIArgumentParser.parseOptions(["in", "out", "--percentage", "50"])
        XCTAssertNil(percentage.width)
        XCTAssertNil(percentage.height)

        let square = try CLIArgumentParser.parseOptions(["in", "out", "--square"])
    ...
    func testFormatFlagParses() throws {
        XCTAssertEqual(try CLIArgumentParser.parseOptions(["in", "out", "--width", "20", "--height", "18", "--format", "png"]).outputFormat, .png)
        XCTAssertEqual(try CLIArgumentParser.parseOptions(["in", "out", "--width", "20", "--height", "18", "--format", "jpeg"]).outputFormat, .jpeg)
        XCTAssertEqual(try CLIArgumentParser.parseOptions(["in", "out", "--width", "20", "--height", "18", "--format", "jpg"]).outputFormat, .jpeg)
        XCTAssertEqual(try CLIArgumentParser.parseOptions(["in", "out", "--width", "20", "--height", "18", "--format", "BMP"]).outputFormat, .bmp)
    }

    func testInvalidFormatThrows() {
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--width", "20", "--height", "18", "--format", "tiff"]))
    }
    ...
    func testInvalidConcurrencyThrows() {
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--width", "20", "--height", "18", "--concurrency", "0"]))
    }

    func testInvalidSeamShapeThrows() {
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--width", "20", "--height", "18", "--seam-shape", "curves"]))
    }

    func testInvalidSeamColorThrows() {
        XCTAssertThrowsError(try CLIArgumentParser.parseOptions(["in", "out", "--width", "20", "--height", "18", "--seam-color", "red"]))
    }
