...
- Fixed the Catalyst dark-mode action rendering by replacing disabled `borderedProminent` buttons with an explicit adaptive action style and preventing mask-action labels from wrapping. A dark-mode GUI regression now verifies Export/Resize visibility; iPhone 17 Simulator passes 31 unit tests plus 2 GUI tests. The `LSPrefs`, `FSFindFolder`, ViewBridge, and task-port messages observed during launch are macOS/Xcode service diagnostics; no app-owned exception was found.
