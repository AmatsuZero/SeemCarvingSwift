...
- Tasks 1–17 are implemented in `worktree/seamcarving` (`.worktrees/seam-carving`), with one commit per task from `b66f6ff` through `d5fe2af`.
- Task 18 is **complete**: benchmark targets, report types, runner, CLI, percentage seam parsing, fixture smoke tests, and benchmark documentation are committed as `bd6576a`; phase/resource work is in `5c10649`, review fixes in `8dd44e9`, Metal phase timing follow-ups in `388d7b1`, `98067fe`, `a1a8006`, forward-energy timing in `0bd5d4a`, and dynamic scratch accounting in `681e5d5`. The equivalent sharded Release matrix (same warmup/iteration/configuration as the plan command, run in parallel to avoid serial 4K starvation) is merged and validated at `/tmp/seam-carving-benchmark.json`: 200 rows covering 5 sizes × 5 seam specs × 2 energies × 4 backends, with 10 raw samples, p50/p95, Xcode, and peak scratch for every row.
- Task 19 is **complete for repository-local gates**: README, architecture notes, benchmark-results template, and three DocC articles are committed as `d5c7f69`. DocC, macOS, generic iOS, full iOS Simulator Metal API/GPU validation, SwiftPM TSan, and manually preloaded Xcode Core/Metal TSan bundles all pass. The physical iPhone/iPad release gate remains explicitly pending as required by the plan.
- Remaining work: none for repository-local implementation and verification; the physical iPhone/iPad Metal release gate remains pending by design. The feature worktree is clean; no unrelated files were observed.
...
- [x] **Step 1: Write the package manifest and failing value-type tests**
...
- [x] **Step 2: Run the tests and confirm the expected compile failure**

Run: `swift test --filter ImageAndMaskTests`

Expected: FAIL because `RGBA8Image`, `RGBA8`, and `Mask` do not exist.

- [x] **Step 3: Implement the exact core types**
...
- [x] **Step 4: Run tests and package validation**
...
- [x] **Step 1: Add hand-calculated failing tests**
...
- [x] **Step 3: Implement the canonical formula**
...
- [x] **Step 1: Add exact-path failing tests**
...
- [x] **Step 3: Implement two-row accumulation plus Int8 parents**
...
- [x] **Step 4: Add and pass validity/property loops**
...
- [x] **Step 1: Add failing pixel-order tests**
...
- [x] **Step 3: Implement gather-based removal and transpose**
...
- [x] **Step 4: Verify transpose equivalence**
...
- [x] **Step 1: Write failing shrink and validation tests**
...
- [x] **Step 3: Implement exact sequential shrinking**
...
- [x] **Step 1: Add failing hand-calculated recurrence tests**
...
- [x] **Step 3: Implement the recurrence**
...
- [x] **Step 4: Verify both energy modes**
...
- [x] **Step 1: Add failing mask behavior tests**
...
- [x] **Step 3: Implement finite and hard adjustments**
...
- [x] **Step 4: Run mask tests and all core tests**
...
- [x] **Step 1: Add failing insertion and mapping tests**
...
- [x] **Step 3: Implement exact discovery with an index map**
...
- [x] **Step 4: Enable general resize and object-size restoration**
...
- [x] **Step 1: Enable the Apple target and add failing round-trip tests**
...
- [x] **Step 3: Implement explicit rendering into straight RGBA8**
...
- [x] **Step 4: Verify bridge and end-to-end resizing**
...
- [x] **Step 1: Enable target and write parity tests**
...
- [x] **Step 3: Implement reusable planar Float buffers**
...
- [x] **Step 4: Verify exact paths and tolerance**
...
- [x] **Step 1: Add the Metal target and a failing passthrough-kernel test**
...
- [x] **Step 3: Implement one-time shader compilation and pipeline caching**
...
- [x] **Step 4: Verify success, error propagation, and repeated cache use**
...
- [x] **Step 1: Add failing stage-parity tests**
...
- [x] **Step 3: Implement kernels with bounds checks**
...
- [x] **Step 4: Implement hybrid orchestration and parity**
...
- [x] **Step 1: Add failing DP parity tests**
...
- [x] **Step 3: Implement row dispatch, reduction, and one-thread backtrack**
...
- [x] **Step 4: Add forward-luma Metal recurrence**
...
- [x] **Step 5: Verify full end-to-end parity and commit**
...
- [x] **Step 1: Add failing orientation/extent/pixel-format tests**
...
- [x] **Step 3: Implement cached contexts and explicit normalization**
...
- [x] **Step 1: Add failing fake-detector and coordinate tests**
...
- [x] **Step 3: Implement the fixed pipeline and policies**
...
- [x] **Step 4: Verify cadence, precedence, and package boundaries**
...
- [x] **Step 1: Write failing selector/fallback tests with fake backends**
...
- [x] **Step 3: Implement an internal injectable selector**
...
- [x] **Step 4: Run concurrency stress tests**
...
- [x] **Step 1: Add a failing CLI smoke test to the executable target**
...
- [x] **Step 3: Implement Foundation-only parsing and ImageIO I/O**
...
- [x] **Step 4: Run an end-to-end generated-image smoke test**
...
- [x] **Step 1: Add benchmark fixture generation and signposts**
...
- [x] **Step 2: Verify benchmarks execute in Release configuration**
...
- [x] **Step 3: Document the benchmark matrix and automatic-policy rule**
...
- [x] **Step 4: Capture a local baseline without changing policy constants**
...
Expected: exit 0. The exact matrix was executed as four-backend parallel shards per image size to avoid serial 4K starvation, then schema-validated and merged into `/tmp/seam-carving-benchmark.json`; do not commit machine-specific numbers. The physical-device release gate still requires a real iPhone/iPad run.
...
- [x] **Step 1: Write user-facing examples and limitations**
...
- [x] **Step 2: Run documentation and formatting sanity checks**
...
- [x] **Step 3: Verify macOS and iOS builds**
...
- [x] **Step 4: Run executable simulator/Metal gates and record the external device gate**
...
- [x] **Step 5: Commit the final documentation**
...
- [x] Every task has its own commit and review gate.
- [x] `git status --short` contains no unrelated changes.
- [x] `git diff --check` passes.
- [x] Debug and Release test suites pass.
- [x] macOS and generic iOS builds pass.
- [x] Metal tests either pass on supported hardware or explicitly skip only because no Metal device exists.
- [x] Metal API Validation and Thread Sanitizer report no issues.
- [x] The benchmark execution report records hardware, OS, Xcode, Swift, backend, image dimensions, seam count, p50/p95, and peak memory.
- [x] `grep -R "import Vision" Sources/SeamCarvingCore` returns no matches; Vision tests record the explicit face-request revision and both cadence call counts.
- [x] Publication remains marked pending until the project owner selects and adds a license; this implementation plan does not create one.
- [x] A separate reviewer checks spec compliance and public API consistency before merge.
